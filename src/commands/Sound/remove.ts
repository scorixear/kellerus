...
export default class Remove extends AutocompleteInteractionModel {
  constructor() {
    super(
      'remove',
      ...
      [
        new SlashCommandStringOption()
          .setName('title')
          .setDescription(LanguageHandler.language.commands.remove.options.title)
          .setRequired(true)
          .setAutocomplete(true),
        new SlashCommandStringOption()
          .setName('category')
          .setDescription(LanguageHandler.language.commands.remove.options.category)
          .setRequired(false)
      ]
    );
  }
  ...
  override async handle(interaction: ChatInputCommandInteraction) {
    try {
      ...
      try {
        if (category && category.trim() !== '' && fs.existsSync('./resources/soundeffects/' + category + '/' + title)) {
          ...
          await MessageHandler.reply({
            ...
            description: LanguageHandler.replaceArgs(LanguageHandler.language.commands.remove.success, [
          ...
          const folders = fs
            ...
            .filter((x) => fs.lstatSync('./resources/soundeffects/' + x).isDirectory());
        ...
        Logger.exception('Error removing sound', err, WARNINGLEVEL.ERROR);
        await MessageHandler.replyError({
          interaction,
          title: LanguageHandler.language.general.error,
      ...
      Logger.exception('Remove Unexpected Error', err, WARNINGLEVEL.ERROR);
      await MessageHandler.replyError({
        interaction,
        title: LanguageHandler.language.general.error,
